...
const OrgSwitcher: React.FunctionComponent<Props> = props => {
  ...
  let { organisationName, email, organisations } = login;
  ...
  let selectOrg = (org: OrganisationFlat) => {
    setSwitcherActive(false);
    dispatch(LoginAction.updateActiveOrgRequest(org));
  };
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <div style={{ margin: "auto 0", maxWidth: "100px" }}>
            <BoldedNavBarHeaderText>{organisationName}</BoldedNavBarHeaderText>
      ...
      >
        ...
        {orgs.map(org => {
          return (
            <DropdownContentText key={org.id} onClick={() => selectOrg(org)}>
              {org.name}
            </DropdownContentText>
          );
        })}
